...
fromCuspsList (_:i:ii:iii:iv:v:vi:vii:viii:ix:x:xi:xii:xs) = Cusps i ii iii iv v vi vii viii ix x xi xii
fromCuspsList _ = error "Invalid cusps list"

fromAnglesList :: [Double] -> Angles
fromAnglesList (a:mc:armc:v:ea:cak:cam:pa:_:_) = Angles a mc armc v ea cak cam pa
...
calculateCusps time loc sys = unsafePerformIO $
    allocaArray 13 $ \cusps ->
        allocaArray 10 $ \ascmc -> do
            ...
            return ((fromCuspsList $ map realToFrac $ cuspsL), (fromAnglesList $ map realToFrac $ anglesL))
